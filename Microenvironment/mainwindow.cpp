...
void MainWindow::save()
{
    ...
    xmlWriter.writeStartElement("microenvironment_setup");
    for(int i = 0; i < substrates.size(); i++){
        xmlWriter.writeStartElement("variable");
        xmlWriter.writeAttribute("name", substrates[i].name);
        xmlWriter.writeAttribute("ID", QString::number(substrates[i].id));
        xmlWriter.writeStartElement("physical_parameter_set");
        xmlWriter.writeTextElement("diffusion_coefficient", QString::number(substrates[i].diffCoeff));
        xmlWriter.writeTextElement("decay_rate", QString::number(substrates[i].decayR));
        xmlWriter.writeEndElement();
        xmlWriter.writeTextElement("initial_condition", QString::number(substrates[i].initialC));
        xmlWriter.writeStartElement("Dirichlet_boundary_condition");
        xmlWriter.writeAttribute("enabled", "false");
        xmlWriter.writeCharacters(QString::number(substrates[i].dirch[0][0]));
        xmlWriter.writeEndElement();
        xmlWriter.writeEndElement();
    }
